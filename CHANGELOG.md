...
### Added
- `days/2026/07/2026-07-07_KongTuke-Mistic-MTLBackdoor-IAB-ClickFix/` — Symantec Threat Hunter Team (2026-06-24), Zscaler ThreatLabz and The Hacker News (2026-06-25) on Backdoor.Mistic (Zscaler: MTLBackdoor), a stealthy new backdoor deployed since April 2026 and linked with medium confidence to the initial access broker Woodgnat (aka KongTuke, 404 TDS, TAG-124, LandUpdate808). Woodgnat sells durable access — not payload — to ransomware affiliates (Qilin, Interlock, Rhysida, Akira, 8Base, Black Basta). Chain: compromised-WordPress TDS serves ClickFix/FileFix/CrashFix lures, a paste-and-run PowerShell pulls a remote MSI, signed `MpExtMs.exe` side-loads `version.dll` (hooks GetModuleFileNameW/LoadLibraryW) which loads `EndpointDlp.dll` = Mistic (in-memory, BOF-loading, self-delete kill switch); recon + fake-login credential theft + Kerberoasting; ModeloRAT WinPython Run-key persistence masquerading as AnyDesk/Splashtop before the access handoff. Tuesday crime-economy slot; first repo primary in slot #4 (IAB) and first case anchored on Woodgnat/KongTuke, Mistic/MTLBackdoor and ModeloRAT.
- Sigma (3): `01_mpextms_sideload_nonsystem_dll.yml` — signed MpExtMs.exe loading version.dll/EndpointDlp.dll from a non-System32 path; `02_clickfix_rundialog_powershell_msi.yml` — explorer.exe-spawned PowerShell/curl/certutil fetching a remote .msi; `03_modelorat_runkey_masquerade.yml` — HKCU Run key named AnyDesk/Splashtop/Comms launching portable pythonw.exe.
- KQL (4): XDR sideload (DeviceImageLoadEvents); ClickFix-to-MSI (DeviceProcessEvents); ModeloRAT persistence (DeviceRegistryEvents); Mistic/Woodgnat C2 callback (DeviceNetworkEvents).
- YARA (1 file, 3 rules): `mistic_woodgnat.yar` — loader API-hooks, Mistic in-memory/BOF markers, .NET fake-lockscreen stealer; behaviour-anchored, filesize-bounded.
- Suricata (1 file, 9 sids): `kongtuke_mistic.rules` — DNS/TLS/HTTP/IP anchors for delivery + C2.
- PEAK hunts (3): ClickFix paste-and-run to MSI; signed-binary non-system DLL side-load (generalised); ModeloRAT WinPython persistence + DGA beacon.
- `iocs.csv` (47 entries) — 9 SHA256, 4 C2/delivery IPs, ~26 domains/subdomains + DGA candidates, delivery URL, strings, regkey, context notes; 2026-06 reporting. No CVE in the chain, so no `kev.md` is generated (expected — the intrusion abuses ClickFix + DLL side-loading, not a vuln).
- `kill_chain.svg` — template A, canonical palette, malware-re accent; victim lane (lure->paste->MSI->sideload->Mistic->recon->persist/handoff) and Woodgnat infrastructure lane, IOC anchors.

### Pedagogy
- Hunt the shape, not the hash: a signed binary loading a system-named DLL from a user-writable path is a portable, hash-independent tell for the whole side-load class.
- In-memory + kill switch changes IR order: capture RAM before isolating/killing, or file scans miss the backdoor.
- An IAB foothold is a pre-ransomware event: defend the intrusion, not the ransomware brand — the same access can end in any of six RaaS families.
- Masquerade lives in the name, not the binary: `EndpointDlp.dll` and a Run key named "AnyDesk" both weaponise naming trust; validate path + signer + data.
...
- `iocs.csv` — 47 entries covering all six payload hashes in SHA256, MD5, 